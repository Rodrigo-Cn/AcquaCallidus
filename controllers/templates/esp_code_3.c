...
volatile unsigned long lastInterruptTimeLCD = 0;

void IRAM_ATTR increase() { pulse++; totalPulses++; }

void IRAM_ATTR isrBotaoLCD() {
  unsigned long currentTime = millis();
  if (currentTime - lastInterruptTimeLCD > 300) {
    botaoLCDPressionado = true;
    lastInterruptTimeLCD = currentTime;
  }
}
...
void exibirTela() {
  ...
  if (rele1Ativo || rele2Ativo || rele3Ativo) {
    lcd.setCursor(0, 0);
    lcd.print("Fluxo Monitorado");
    lcd.setCursor(0, 1);
    lcd.print("V1:");
    lcd.print(totalLitrosV1, 0);
    lcd.print(" V2:");
    lcd.print(totalLitrosV2, 0);
    ...
    lcd.setCursor(0, 1);
    char buffer[17];
    snprintf(buffer, sizeof(buffer), "%02uh %02um", horas, minutos);
    lcd.print(buffer);
  } else {
    lcd.setCursor(0, 0);
...
void loop() {
  ...
  if (currentTime - lastTime >= 1000) {
    ...
    if (rele3Ativo) totalLitrosV3 += litros;
  ...
  if ((millis() >= proximaIrrigacao) || ultimoDia == 0) {
    ...
    totalLitrosV1 = totalLitrosV2 = totalLitrosV3 = 0;
  ...
  if (rele2Ativo && totalLitrosV2 >= metaV2) {
    ...
    metaV3 = getLitrosPorValvula(valve_ids[2]);
    rele3Ativo = true;
    digitalWrite(relePin3, LOW);
    delay(2000);
  }

  if (rele3Ativo && totalLitrosV3 >= metaV3) {
    digitalWrite(relePin3, HIGH);
    rele3Ativo = false;
    cicloDoDiaConcluido = true;
    desligarValvulaNoServidor(valve_ids[2], totalLitrosV3);
    totalLitrosV1 = totalLitrosV2 = totalLitrosV3 = 0;
